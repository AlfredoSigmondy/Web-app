...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $_SESSION['form_data'] = [
        'Username' => $_POST['Username'] ?? $_SESSION['form_data']['Username'],
        'Gender' => $_POST['Gender'] ?? $_SESSION['form_data']['Gender'],
        'Email' => $_POST['Email'] ?? $_SESSION['form_data']['Email'],
        'Birthdate' => $_POST['Birthdate'] ?? $_SESSION['form_data']['Birthdate']
    ];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_otp'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $_SESSION['form_data']['Username'] = $name;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    if (isset($_SESSION['otp_verified']) && $_SESSION['otp_verified'] === true) {
        $name = $_SESSION['form_data']['Username'];
        $gender = $_SESSION['form_data']['Gender'];
        $email = $_SESSION['form_data']['Email'];
        $password = password_hash($_POST['Password'], PASSWORD_DEFAULT);
        $dob = $_SESSION['form_data']['Birthdate'];
        ...
        if ($stmt->execute()) {
            ...
            unset($_SESSION['signup_otp'], $_SESSION['signup_email'], $_SESSION['signup_name'], 
                  $_SESSION['otp_verified'], $_SESSION['otp_sent'], $_SESSION['form_data']);
                ...
                <form method="post" autocomplete="on" id="signupForm">
                        ...
                        <div class="d-flex gap-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="Gender" id="genderMale" 
                                       value="Male" <?= $_SESSION['form_data']['Gender'] == 'Male' ? 'checked' : '' ?> required>
                                <label class="form-check-label" for="genderMale">Male</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="Gender" id="genderFemale" 
                                       value="Female" <?= $_SESSION['form_data']['Gender'] == 'Female' ? 'checked' : '' ?> required>
                                <label class="form-check-label" for="genderFemale">Female</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="Gender" id="genderOther" 
                                       value="Other" <?= $_SESSION['form_data']['Gender'] == 'Other' ? 'checked' : '' ?> required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="Birthdate" class="form-control" 
                               required value="<?= htmlspecialchars($_SESSION['form_data']['Birthdate']) ?>">
                    </div>
                    <?php if ($otpSent): ?>
                        <div class="mb-3">
...
document.getElementById('signupForm')?.addEventListener('submit', function(e) {
    ...
    const form = this;
    const inputs = form.querySelectorAll('input:not([type="submit"]):not([name="otp"]):not([name="Password"])');
...
document.addEventListener('DOMContentLoaded', function() {
    const inputs = document.querySelectorAll('#signupForm input:not([type="submit"]):not([name="otp"]):not([name="Password"])');
    inputs.forEach(input => {
